  ...
  return (
    <nav className="w-full absolute top-0 left-0 z-50 flex items-center sm:justify-start justify-between px-6 py-3 text-primary max-w-screen ss:p-10 p-4 lg:py-8 lg:px-32 md:px-24">
      ...
      <div className="flex items-center">
        <img src="/images/nav-logo.png" alt="hero-logo" />
      </div>
      ...
      <ul className="hidden sm:flex space-x-10 ml-2 lg:ml-24 flex-wrap">
        ...
        <li>
          <Link href="/" className="font-normal text-xl">About</Link>
        </li>
        <li>
          <Link href="/shop" className="font-normal text-xl">Shop</Link>
        </li>
        <li>
          <Link href="/faqs" className="font-normal text-xl">FAQs</Link>
        </li>
      </ul>
      ...
      <div className="block sm:hidden relative text-primary z-50">
        <button className="focus:outline-none" onClick={toggleMenu}>
          {isOpen ? (
            <span className="text-2xl"><MdClose /></span> // Close button
          ) : (
            <span className="text-2xl"><RxHamburgerMenu /></span> // Open button
          )}
        </button>
